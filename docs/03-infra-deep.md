...
| ...with `text/event-stream` | the same work, reported as it happens: `progress` events carrying `{ step, phase, reading, answer, tools }`, then one `answer` event whose body is that same `{ turn, exhausted }`, or one `failure` event carrying `{ error }`. `phase` is `reading`, `looking` or `writing`. Every field is what is known so far rather than what changed, so a client replaces its state per event and never appends. `answer` can come back shorter than the event before it: `claude -p` validates its own structured output and rewrites the whole object when it fails, which a reader sees as the answer starting again. The status is spent when the headers go out, so a failure after that is the `failure` event and never a code |
