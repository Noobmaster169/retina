...
import { type ClassifyJob, type CompareJob, type IngestJob, QUEUES } from "./names";
...
export async function closeQueues(): Promise<void> {
  ...
  queues = undefined;
  await Promise.all([closing.ingest.close(), closing.classify.close(), closing.compare.close()]);
}
