...
/**
 * A model named for one run must be a proxy alias. Checked here, before any
 * email is queued, because a wrong one otherwise fails every email of the run.
 */
...
export async function planRun(db: Queryable, body: CreateRunBody): Promise<RunPlan> {
  const unknown = await unknownModels(Object.values(body.models ?? {}));
