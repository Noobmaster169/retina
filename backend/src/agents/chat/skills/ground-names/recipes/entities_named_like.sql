...
-- version: 2
-- about: Things of one kind with any spelling matching a like pattern, such as a country or a group word. The kind is port, party, carrier, person, commodity or vessel.
...
select en.id as entity_id, en.kind, en.canonical, min(n.value) as matched_spelling,
 ...
 where en.kind = $1::text and en.merged_into is null and n.value ilike $2::text
