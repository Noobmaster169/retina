...
3. Filter on a value exactly as it came back, including its case and punctuation. A value
   longer than 120 characters is shown cut, and the tool says so: reach it with a `like` pattern
   and a `%`, never with `=`.
4. When the value you wanted is not in the list, it may be rare rather than absent: search for
   it with a `like` pattern that has a `%` in it, which is always allowed, and then filter on
   what that returns.
