...
export function ScoreboardView({ board }: { board: Scoreboard }) {
  ...
  return (
    <div>
      <dl className="flex flex-wrap gap-x-10 gap-y-4 border-y border-line py-4">
          ...
          <table className="mt-2 text-right text-xs tabular-nums">
            ...
            <tbody>
              {CATEGORIES.map((actual) => (
                <tr key={actual} className="border-t border-line">
                  ...
                  {CATEGORIES.map((predicted) => {
                    ...
                    const tone = n === 0 ? "text-muted" : actual === predicted ? "text-accent-ink font-semibold" : "text-red-700 font-semibold";
                    return (
        ...
        <div>
          ...
          <ul className="mt-2 text-sm">
            {Object.entries(reliability.per_reason).map(([reason, { total, caught }]) => (
              <li key={reason} className="flex justify-between border-t border-line py-1">
                <span>{reason}</span>
                <span className={`tabular-nums ${caught < total ? "text-red-700" : "text-accent-ink"}`}>
                  {caught} of {total}
                </span>
              </li>
            ))}
