...
async function load(): Promise<Loaded> {
  try {
    const [overview, senders, held] = await Promise.all([
      getGate(),
      listGateSenders(),
      listHeld(),
    ]);
    ...
    return {
      ...
      senders: null,
      held: null,
      backendError: "The backend is not reachable right now.",
    };
  }
}
      ...
      <main className="min-h-0 grow overflow-y-auto px-7 pb-8">
        <div className="py-5">
          ...
          <p className="mt-0.5 max-w-[72ch] text-body text-ink-tertiary">
            Whether an email is worth what reading it will cost, decided by
            arithmetic over how much arrived, how often, and how long we have
            known the sender. It never reads the words in an email and it never
            decides what one is: the model classifies everything that gets
            through, whoever sent it.
          </p>
        </div>
        <GatePanes {...loaded} />
      </main>
