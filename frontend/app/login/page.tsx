...
export default async function LoginPage({ searchParams }: PageProps<"/login">) {
  ...
  return (
    <main className="mx-auto w-full max-w-sm px-5 py-16">
      ...
      <p className="mt-1 text-sm text-ink-tertiary">Asking a model and starting a run both spend shared resources, so those pages need the site password.</p>
      <form action={login} className="mt-6 flex flex-col gap-4">
        ...
        {error && <p className="text-sm text-fault">Wrong password.</p>}
        <button
          type="submit"
