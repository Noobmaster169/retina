...
/**
 * A shipping instruction or a bill of lading check can be answered with a
 * draft, and the shipment record is what that draft is allowed to rest on.
 *
...
export function DraftShipmentButton({
  ...
  const dock = useDock();
  const what = category === "SI_REQUEST" ? "shipping instruction" : "bill of lading check";
  return (
    <Button
      variant={variant}
      onClick={() =>
